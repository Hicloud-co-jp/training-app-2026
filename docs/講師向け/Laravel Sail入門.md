...
この資料は講師向けです。Laravel Sailが何をしているかを、Dockerを使い始めたばかりでも理解できる言葉で体系的に説明します。

新入社員は日々の簡単な操作を優先し、PowerShellからDocker Composeを使用します。講師はUbuntuとSailを使用して、その裏側にあるコンテナ構成やコマンドの対応まで学びます。どちらも同じ`compose.yaml`とコンテナを操作します。
...
新入社員のPCを起動するだけの場合は「新入社員向け環境構築」を使用してください。この資料のUbuntu導入は、Sailを学ぶ講師用です。
