...
<div class="row g-3">
    <div class="col-md-6">
        <label class="form-label" for="employee_number">
            社員番号 <span class="text-danger">*</span>
        </label>
        <input
    ...
    <div class="col-md-6">
        <label class="form-label" for="joined_at">入社日</label>
        <input
            class="form-control @error('joined_at') is-invalid @enderror"
            id="joined_at"
            name="joined_at"
            type="date"
            value="{{ old('joined_at', isset($employee) ? $employee->joined_at?->format('Y-m-d') : '') }}"
        >
        @error('joined_at')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
...
<div class="d-flex justify-content-end gap-2 mt-4">
    <a class="btn btn-outline-secondary" href="{{ route('employees.index') }}">キャンセル</a>
